...
-(void)addObserverName:(NSString*)name object:(id)obj handler:(StatusBlock)handler
{
    ...
    //DLog(@"Add Observer:%@ %@", name, self.handlerDictionary);
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(receiveObserverNoti:) name:name object:obj];
}
...
-(void)removeHandlerObserverWithName:(NSString *)name object:(id)obj
{
    ...
    DLog(@"Remove Observer:%@ %@", name, self.handlerDictionary);
}
